...
const handleSendResponse = async () => {
  ...
  <button
    style={{
      ...replyBtn,
      backgroundColor: report.is_respond === true ? "#ccc" : replyBtn.backgroundColor,
      cursor: report.is_respond === true ? "none" : "pointer",
    }}
    onClick={() => {
      if (!report.is_respond === true) {
        openModal(report);
      }
    }}
    disabled={!!report.is_respond === true}
  >
    <FiSend style={{ marginRight: "5px" }} />
    {report.is_respond === true ? "Responded" : "Respond"}
  </button>
</div>
